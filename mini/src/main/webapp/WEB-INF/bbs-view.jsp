			...
			<div v-for="(item, index) in commentList" style="font-size : 1.5em; margin : 10px;">
				<div v-if="item.commentNo != commentNo">
					<span v-if="item.delYn == 'N'">
						{{item.id}}({{item.cdatetime}}) : <pre class="commentContents">{{item.content}}</pre>
...
<script type="text/javascript">
	var app = new Vue({
		...
		data : {
			list : [],
			info : {},
			boardNo : "${map.boardNo}", //controller에서 보내준 맵은 $로 접근이 가능함.
			updateFlg : true,
			title : '',
			content : '',
			sessionId : "${sessionId}",
		...
		methods : {
			...
			fnRemoveComment(item){
				var self = this;
				var nparmap = item;
				$.ajax({
					url : "/bbs/removeComment.dox",
					dataType : "json",
					type : "POST",
					data : nparmap,
					success : function(data) {
